...
export async function parseKill(event: string) {
    ...
    let killData = JSON.parse(event).package;
    // Hack to get into correct format after migration from websocket. Disgusting and needs to be fixed
    killData.killmail.zkb = killData.zkb
    killData = killData.killmail

    if (!isFriendlyKill(killData) && isKillInChain(killData) && isWormholeKill(killData) && isTodayKill(killData)) {
